...
class FormulaireExp42AController extends Controller
{
    public function index()
    {
        ...
        return view('formulaire_exp_4_2_a.index', compact('formulaires'));
    }
    ...
    public function create()
    {
        ...
        return view('formulaire_exp_4_2_a.create', compact('dossiers', 'entreprises', 'signataires'));
    }
    ...
    public function generateForm(Request $request)
    {
        ...
        return view('formulaire_exp_4_2_a.generate', [
    ...
    public function store(Request $request)
    {
        ...
        $validated['formulaire_type'] = 'A';
        $validated['montant_part'] = $this->computeMontantPart($validated);

        FormulaireExp42A::create($validated);

        return redirect()->route('formulaire_exp_4_2_a.index')->with('success', 'Formulaire EXP-4.2 a) créé avec succès.');
    }

    public function edit(FormulaireExp42A $formulaireExp42A)
    {
        $this->authorizeForm($formulaireExp42A);

        $dossiers = Dossier::with('entreprise')->orderByDesc('created_at')->get();
        $entreprises = Entreprise::orderBy('nom')->get();
        $signataires = Signataire::orderBy('nom')->orderBy('prenom')->get();

        return view('formulaire_exp_4_2_a.edit', compact('formulaireExp42A', 'dossiers', 'entreprises', 'signataires'));
    }

    public function update(Request $request, FormulaireExp42A $formulaireExp42A)
    {
        $this->authorizeForm($formulaireExp42A);
        ...
        $formulaireExp42A->update($validated);

        return redirect()->route('formulaire_exp_4_2_a.index')->with('success', 'Formulaire EXP-4.2 a) mis à jour avec succès.');
    }
    ...
    public function generate(Request $request)
    {
        ...
        $formulaireExp42A = FormulaireExp42A::create($validated);
        $formulaireExp42A->loadMissing('entreprise', 'signataire');
        $dossier = !empty($validated['dossier_id']) ? Dossier::with('entreprise', 'signataires')->find($validated['dossier_id']) : null;

        // Construire logoDataUri (priorité logo entreprise puis fallbacks)
        $logoDataUri = '';
        $logo = optional($dossier->entreprise)->logo ?? optional($formulaireExp42A->entreprise)->logo ?? null;
        if ($logo) {
        ...
        $html = view('formulaire_exp_4_2_a.pdf', compact('formulaireExp42A', 'dossier', 'logoDataUri'))->render();
        ...
        $filename = 'formulaire_exp_4_2_a_' . $formulaireExp42A->id . '.pdf';
    ...
    public function download(FormulaireExp42A $formulaireExp42A)
    {
        $this->authorizeForm($formulaireExp42A);

        $formulaireExp42A->loadMissing('entreprise', 'signataire', 'dossier.entreprise', 'dossier.signataires');
        $dossier = $formulaireExp42A->dossier;
    ...
    public function destroy(FormulaireExp42A $formulaireExp42A)
    {
        $this->authorizeForm($formulaireExp42A);

        $formulaireExp42A->delete();

        return redirect()->route('formulaire_exp_4_2_a.index')->with('success', 'Formulaire EXP-4.2 a) supprimé avec succès.');
    }
