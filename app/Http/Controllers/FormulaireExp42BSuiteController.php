...
class FormulaireExp42BSuiteController extends Controller
{
    public function index()
    {
        ...
        return view('formulaire_exp_4_2_b_suite.index', compact('formulaires'));
    }
    ...
    public function create()
    {
        ...
        return view('formulaire_exp_4_2_b_suite.create', compact('dossiers', 'entreprises', 'signataires'));
    }
    ...
    public function generateForm(Request $request)
    {
        ...
        return view('formulaire_exp_4_2_b_suite.generate', [
    ...
    public function store(Request $request)
    {
        ...
        $validated['formulaire_type'] = 'B_SUITE';

        FormulaireExp42BSuite::create($validated);

        return redirect()->route('formulaire_exp_4_2_b_suite.index')->with('success', 'Formulaire EXP-4.2 b) (suite) créé avec succès.');
    }

    public function edit(FormulaireExp42BSuite $formulaireExp42BSuite)
    {
        $this->authorizeForm($formulaireExp42BSuite);

        $dossiers = Dossier::with('entreprise')->orderByDesc('created_at')->get();
        $entreprises = Entreprise::orderBy('nom')->get();
        $signataires = Signataire::orderBy('nom')->orderBy('prenom')->get();

        return view('formulaire_exp_4_2_b_suite.edit', compact('formulaireExp42BSuite', 'dossiers', 'entreprises', 'signataires'));
    }

    public function update(Request $request, FormulaireExp42BSuite $formulaireExp42BSuite)
    {
        $this->authorizeForm($formulaireExp42BSuite);
        ...
        $formulaireExp42BSuite->update($validated);

        return redirect()->route('formulaire_exp_4_2_b_suite.index')->with('success', 'Formulaire EXP-4.2 b) (suite) mis à jour avec succès.');
    }
    ...
    public function generate(Request $request)
    {
        ...
        $formulaireExp42BSuite = FormulaireExp42BSuite::create($validated);
        $formulaireExp42BSuite->loadMissing('entreprise', 'signataire');
        $dossier = !empty($validated['dossier_id']) ? Dossier::with('entreprise', 'signataires')->find($validated['dossier_id']) : null;

        $logoDataUri = $this->getLogoDataUri($dossier, $formulaireExp42BSuite);

        $html = view('formulaire_exp_4_2_b_suite.pdf', compact('formulaireExp42BSuite', 'dossier', 'logoDataUri'))->render();
        ...
        $filename = 'formulaire_exp_4_2_b_suite_' . $formulaireExp42BSuite->id . '.pdf';
    ...
    public function download(FormulaireExp42BSuite $formulaireExp42BSuite)
    {
        $this->authorizeForm($formulaireExp42BSuite);

        $formulaireExp42BSuite->loadMissing('entreprise', 'signataire', 'dossier.entreprise', 'dossier.signataires');
        $dossier = $formulaireExp42BSuite->dossier;
    ...
    public function destroy(FormulaireExp42BSuite $formulaireExp42BSuite)
    {
        $this->authorizeForm($formulaireExp42BSuite);

        $formulaireExp42BSuite->delete();

        return redirect()->route('formulaire_exp_4_2_b_suite.index')->with('success', 'Formulaire EXP-4.2 b) (suite) supprimé avec succès.');
    }
