<div id="bordereau-fournitures-deja-importees-step6">
    @php
        $existingValues = [];
                ...
                return [
                    ...
                    'lignes' => $bordereau->lignes->map(function ($ligne) {
                        return [
                            ...
                            'droits_douane' => $ligne->droits_douane ?? '',
        ...
        if (empty($oldSections)) {
            $oldSections = [
                [
                    'titre' => '',
                    'lignes' => [
                        ['designation' => '', 'site' => '', 'quantite' => '', 'prix_unitaire' => '', 'date_prestation' => '', 'droits_douane' => '', 'transport' => '', 'taxe_vente' => ''],
                    ],
                ],
            ];
        }
    ...
    <div class="wizard-field" style="margin-bottom: 16px;">
        <p class="wizard-label">Bordereau des prix des fournitures, déjà importées</p>
    </div>
                    ...
                    <table class="table" style="width:100%; border-collapse:collapse; font-size:10px;">
                        <thead>
                            <tr>
                                ...
                                <th style="border:1px solid #ccc; padding:5px; width:13%;">2<br>Description</th>
                                <th style="border:1px solid #ccc; padding:5px; width:8%;">3<br>Pays d'origine</th>
                                <th style="border:1px solid #ccc; padding:5px; width:9%;">4<br>Date de livraison (Incoterms)</th>
                                <th style="border:1px solid #ccc; padding:5px; width:6%;">5<br>Quantité</th>
                                <th style="border:1px solid #ccc; padding:5px; width:9%;">6<br>Prix unit. incl. droits/taxes</th>
                                <th style="border:1px solid #ccc; padding:5px; width:8%;">7<br>Droits/taxes par unité</th>
                                <th style="border:1px solid #ccc; padding:5px; width:8%;">8<br>Prix unit. net (6-7)</th>
                                <th style="border:1px solid #ccc; padding:5px; width:9%;">9<br>Prix article net (5×8)</th>
                                <th style="border:1px solid #ccc; padding:5px; width:9%;">10<br>Prix transport</th>
                                <th style="border:1px solid #ccc; padding:5px; width:8%;">11<br>Taxes de vente</th>
                                <th style="border:1px solid #ccc; padding:5px; width:7%;">12<br>Prix total (9+10)</th>
                        ...
                        <tbody class="bordereau-lines">
                            @foreach($section['lignes'] as $lineIndex => $ligne)
                                <tr>
                                    ...
                                    <td style="border:1px solid #ccc; padding:5px; vertical-align:top;">
                                        <input type="text" name="sections[{{ $sectionIndex }}][lignes][{{ $lineIndex }}][site]" class="form-control wizard-input" value="{{ $ligne['site'] ?? '' }}" autocomplete="new-password" data-lpignore="true" data-form-type="other">
                                    </td>
                                    <td style="border:1px solid #ccc; padding:5px; vertical-align:top;">
                                    ...
                                    <td style="border:1px solid #ccc; padding:5px; vertical-align:top;">
                                        <input type="text" name="sections[{{ $sectionIndex }}][lignes][{{ $lineIndex }}][droits_douane]" class="form-control wizard-input droits-input" value="{{ $ligne['droits_douane'] ?? '' }}">
                                    </td>
                                    <td style="border:1px solid #ccc; padding:5px; vertical-align:top;">
                                        <input type="text" class="form-control wizard-input prix-net-display" placeholder="Auto-calc" disabled value="">
                                    </td>
                                    <td style="border:1px solid #ccc; padding:5px; vertical-align:top;">
                                    ...
                                    <td style="border:1px solid #ccc; padding:5px; vertical-align:top;">
                                        <input type="text" class="form-control wizard-input prix-total-display" placeholder="Auto-calc" disabled value="">
                                    </td>
    ...
    <div class="mb-3" style="font-size:0.85rem; color:#555;">
        Les colonnes 8 (Prix unitaire net), 9 (Prix article net) et 12 (Prix total par article) se calculent automatiquement (8 = 6-7, 9 = 5×8, 12 = 9+10).
    </div>
</div>
    ...
    (function () {
        ...
        function createLineRow(sectionIndex) {
            ...
            row.innerHTML = `
                ...
                <td style="border:1px solid #ccc; padding:5px; vertical-align:top;">
                    <input type="text" name="sections[${sectionIndex}][lignes][0][site]" class="form-control wizard-input" autocomplete="new-password" data-lpignore="true" data-form-type="other">
                </td>
                <td style="border:1px solid #ccc; padding:5px; vertical-align:top;">
                ...
                <td style="border:1px solid #ccc; padding:5px; vertical-align:top;">
                    <input type="text" name="sections[${sectionIndex}][lignes][0][droits_douane]" class="form-control wizard-input droits-input">
                </td>
                <td style="border:1px solid #ccc; padding:5px; vertical-align:top;">
                    <input type="text" class="form-control wizard-input prix-net-display" placeholder="Auto-calc" disabled value="">
                </td>
        ...
        function updateRowTotals(row) {
            ...
            const droitsInput = row.querySelector('input[name*="[droits_douane]"]');
            const quantiteInput = row.querySelector('input[name*="[quantite]"]');
            const transportInput = row.querySelector('input[name*="[transport]"]');
            const prixNetDisplay = row.querySelector('input.prix-net-display');
            ...
            const droits = Number(droitsInput?.value || 0);
            const quantite = Number(quantiteInput?.value || 0);
            const transport = Number(transportInput?.value || 0);
            const prixNet = price - droits;
            const montant = prixNet * quantite;

            if (prixNetDisplay) {
                prixNetDisplay.value = (price || droits) ? prixNet : '';
            }
            if (montantDisplay) {
                montantDisplay.value = montant ? montant : '';
            }
            if (prixTotalDisplay) {
                const total = montant + transport;
        ...
        sectionsContainer.addEventListener('input', (event) => {
            const row = event.target.closest('tr');
            if (!row) return;
            if (event.target.name.includes('[prix_unitaire]') || event.target.name.includes('[droits_douane]') || event.target.name.includes('[quantite]') || event.target.name.includes('[transport]')) {
                updateRowTotals(row);
            }
        });
        ...
        if (window.DaoTableImport) {
            window.DaoTableImport.setup({
                ...
                fieldSynonyms: {
                    ...
                    site: ['pays d origine', 'pays'],
                    date_prestation: ['date de livraison', 'date'],
                    quantite: ['quantite', 'qte'],
                    prix_unitaire: ['prix unitaire', 'prix unitaire incluant droits'],
                    droits_douane: ['droits de douane', 'droits douane', 'taxes d importation'],
