<div id="calendrier-step6">
        ...
        if (!empty($oldSections)) {
            foreach ($oldSections as $sKey => $sec) {
                if (!isset($oldSections[$sKey]['lignes']) || !is_array($oldSections[$sKey]['lignes'])) {
                    $oldSections[$sKey]['lignes'] = [ ['designation' => '', 'date_prestation' => '', 'quantite' => 1, 'prix_unitaire' => '', 'montant' => ''] ];
                    continue;
                }
        ...
        if (empty($oldSections)) {
            $oldSections = [
                [
                    'titre' => '',
                    'lignes' => [
                        ['designation' => '', 'date_prestation' => '', 'quantite' => 1, 'prix_unitaire' => '', 'montant' => ''],
                    ],
                ],
            ];
        }
                    ...
                    <table class="table" style="width:100%; border-collapse:collapse; font-size:12px;">
                        <thead>
                            <tr>
                                ...
                                <th style="border:1px solid #ccc; padding:8px; width:40%;">Désignation des produits</th>
                                <th style="border:1px solid #ccc; padding:8px; width:20%;">Date de réalisation au lieu de destination finale</th>
                                <th style="border:1px solid #ccc; padding:8px; width:10%;">Quantité (nb d'unités)</th>
                                <th style="border:1px solid #ccc; padding:8px; width:13%;">Prix unitaire</th>
                                <th style="border:1px solid #ccc; padding:8px; width:12%;">Total par article (Col 4°)</th>
                        ...
                        <tbody class="bordereau-lines">
                            @foreach($section['lignes'] as $lineIndex => $ligne)
                                <tr>
                                    ...
                                    <td style="border:1px solid #ccc; padding:8px; vertical-align:top;">
                                        <input type="text" name="sections[{{ $sectionIndex }}][lignes][{{ $lineIndex }}][prix_unitaire]" class="form-control wizard-input calendrier-price" placeholder="150000" value="{{ $ligne['prix_unitaire'] ?? '' }}">
                                    </td>
                                    <td style="border:1px solid #ccc; padding:8px; vertical-align:top;">
                                        <input type="text" class="form-control wizard-input calendrier-total" value="{{ $ligne['montant'] ?? ($ligne['prix_unitaire'] ?? '') }}" disabled>
                                    </td>
    ...
    <div class="mb-3" style="font-size:0.85rem; color:#555;">
        Ajoutez un ou plusieurs tableaux. Chaque tableau possède son titre propre et ses lignes de désignation, date, quantité et prix unitaire. Le signataire est placé en fin de document dans le PDF.
    </div>
</div>
...
<script>
    (function () {
        ...
        function createLineRow(sectionIndex, designation = '', date = '', quantity = '1', price = '') {
            ...
            row.innerHTML = `
                ...
                <td style="border:1px solid #ccc; padding:8px; vertical-align:top;">
                    <input type="text" name="sections[${sectionIndex}][lignes][0][prix_unitaire]" class="form-control wizard-input calendrier-price" placeholder="150000" value="${price}">
                </td>
                <td style="border:1px solid #ccc; padding:8px; vertical-align:top;">
                    <input type="text" class="form-control wizard-input calendrier-total" value="" disabled>
                </td>
                <td style="border:1px solid #ccc; padding:8px; vertical-align:top; text-align:center;">
                    <button type="button" class="btn btn-sm btn-danger removeBordereauLine" style="padding:0.25rem 0.45rem;">×</button>
                </td>
        ...
        function updateRowTotal(row) {
            ...
            const totalDisplay = row.querySelector('input.calendrier-total');
            const price = parseFloat(priceInput?.value) || 0;
            const quantity = parseInt(quantityInput?.value) || 1;
            const total = price * quantity;
            if (totalDisplay) {
                totalDisplay.value = total > 0 ? total : '';
            }
        }
