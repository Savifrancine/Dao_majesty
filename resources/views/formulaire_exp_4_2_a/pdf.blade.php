...
<body>
    ...
    <div class="page">
        ...
        <div class="title-section">FORMULAIRE EXP-4.2 a)</div>
        <div class="subtitle">Expérience spécifique de fournitures/services</div>

    @php
        $dossier = $dossier ?? ($formulaireExp42A->dossier ?? null);
        $entreprise = $formulaireExp42A->entreprise ?? optional($dossier)->entreprise;

        $companyName = trim(
            optional($entreprise)->nom
            ?? optional($entreprise)->responsable
            ?? $formulaireExp42A->nom_candidat
            ?? ''
        );
        ...
        $autorite_nom = trim($formulaireExp42A->autorite_nom ?? '');
        $autorite_adresse = trim($formulaireExp42A->autorite_adresse ?? '');
        $autorite_telephone = trim($formulaireExp42A->autorite_telephone ?? '');
        $autorite_email = trim($formulaireExp42A->autorite_email ?? '');

        $signataire = $formulaireExp42A->signataire ?? null;
        if (!$signataire) {
    ...
    <table class="block-table">
        ...
        <tr>
            ...
            <td colspan="3" style="vertical-align:top;">{!! nl2br(e($formulaireExp42A->identification_marche ?? '')) !!}</td>
        </tr>
        <tr>
        ...
        @php
            $montantTotal = '';
            if (!empty($formulaireExp42A->montant_total)) {
                $montantTotal = number_format((float) str_replace(["\xc2\xa0"," ",','], ['', '', '.'], $formulaireExp42A->montant_total), 0, ',', ' ');
            }
        @endphp
        <tr>
            <td><strong>Rôle dans le marché</strong></td>
            <td style="width:21%; text-align:center;">
                <div style="display:inline-block; width:14px; height:14px; border:1px solid #000; margin-bottom:4px; line-height:14px; font-size:10px; text-align:center; font-family: DejaVu Sans, Arial, sans-serif;">{{ trim($formulaireExp42A->role_marche) === 'Fournisseur/Prestataire' ? '✔' : '' }}</div>
                <div style="font-size:10px; line-height:1.1;">Fournisseur/<br>Prestataire</div>
            </td>
            <td style="width:21%; text-align:center;">
                <div style="display:inline-block; width:14px; height:14px; border:1px solid #000; margin-bottom:4px; line-height:14px; font-size:10px; text-align:center; font-family: DejaVu Sans, Arial, sans-serif;">{{ trim($formulaireExp42A->role_marche) === 'Ensemblier' ? '✔' : '' }}</div>
                <div style="font-size:10px; line-height:1.1;">Ensemblier</div>
            </td>
            <td style="width:23%; text-align:center;">
                <div style="display:inline-block; width:14px; height:14px; border:1px solid #000; margin-bottom:4px; line-height:14px; font-size:10px; text-align:center; font-family: DejaVu Sans, Arial, sans-serif;">{{ trim($formulaireExp42A->role_marche) === 'Sous-traitant' ? '✔' : '' }}</div>
        ...
        <tr>
            ...
            <td style="text-align:center; font-weight:700;">{{ $formulaireExp42A->monnaie ?? 'FCFA' }}</td>
        </tr>
        <tr>
            ...
            <td style="text-align:center;">{{ $formulaireExp42A->participation_pourcentage ? $formulaireExp42A->participation_pourcentage . ' %' : 'NEANT' }}</td>
            <td style="text-align:center;">{{ $formulaireExp42A->montant_part ? number_format((float) str_replace(["\xc2\xa0"," ",','], ['', '', '.'], $formulaireExp42A->montant_part), 0, ',', ' ') : 'NEANT' }}</td>
            <td style="text-align:center;">{{ $formulaireExp42A->monnaie ?? 'FCFA' }}</td>
        </tr>
        <tr>
