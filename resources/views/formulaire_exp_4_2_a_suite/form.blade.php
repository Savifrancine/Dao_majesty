@php
    ...
    $selectedEntrepriseId = old('entreprise_id', optional($formulaireExp42ASuite)->entreprise_id ?? optional($dossier)->entreprise->id ?? '');
    $selectedDossierId = old('dossier_id', $dossier_id ?? $formulaireExp42ASuite->dossier_id ?? '');
    $selectedSignataireId = old('signataire_id', $formulaireExp42ASuite->signataire_id ?? '');
@endphp
...
<div style="margin-top:10px;">
    <form action="{{ $action }}" method="{{ in_array($method, ['PUT','PATCH']) ? 'POST' : $method }}">
        ...
        <div style="font-weight:700; font-size:14px; margin:16px 0 12px 0; text-align:center; background:#f3f4f6; padding:8px; border-radius:4px;">
            {{ $formTitle ?? 'FORMULAIRE EXP-4.2 a) (suite)' }}
        </div>

        <div class="wizard-field" style="margin-bottom:18px;">
            <label class="wizard-label">Numéro de marché similaire</label>
            <input type="text" name="numero_marche" class="form-control" placeholder="Numéro de marché similaire" value="{{ old('numero_marche', optional($formulaireExp42ASuite)->numero_marche ?? '') }}">
        </div>

        <div style="width:100%; overflow-x:auto;">
            <table id="exp42-a-suite-table" style="width:100%; border-collapse:collapse; font-size:13px;">
            <thead>
            ...
            <tbody>
                <tr>
                    <td style="border:1px solid #000; padding:8px; vertical-align:top;">
                        <textarea name="description_similitude" class="form-control form-control-sm" rows="3" placeholder="Description" style="font-size:12px;">{{ old('description_similitude', optional($formulaireExp42ASuite)->description_similitude ?? '') }}</textarea>
                    </td>
                    <td style="border:1px solid #000; padding:8px; vertical-align:top;">
                        <input type="text" name="montant" class="form-control form-control-sm" placeholder="Montant" value="{{ old('montant', optional($formulaireExp42ASuite)->montant ?? '') }}" style="font-size:12px;">
                    </td>
                    <td style="border:1px solid #000; padding:8px; vertical-align:top;">
                        <input type="text" name="taille_physique" class="form-control form-control-sm" placeholder="Taille physique" value="{{ old('taille_physique', optional($formulaireExp42ASuite)->taille_physique ?? '') }}" style="font-size:12px;">
                    </td>
                    <td style="border:1px solid #000; padding:8px; vertical-align:top;">
                        <input type="text" name="complexite" class="form-control form-control-sm" placeholder="Complexité" value="{{ old('complexite', optional($formulaireExp42ASuite)->complexite ?? '') }}" style="font-size:12px;">
                    </td>
                    <td style="border:1px solid #000; padding:8px; vertical-align:top;">
                        <input type="text" name="methodes_technologie" class="form-control form-control-sm" placeholder="Méthodes" value="{{ old('methodes_technologie', optional($formulaireExp42ASuite)->methodes_technologie ?? '') }}" style="font-size:12px;">
                    </td>
                    <td style="border:1px solid #000; padding:8px; vertical-align:top;">
                        <input type="text" name="autres_caracteristiques" class="form-control form-control-sm" placeholder="Autres" value="{{ old('autres_caracteristiques', optional($formulaireExp42ASuite)->autres_caracteristiques ?? '') }}" style="font-size:12px;">
                    </td>
                </tr>
            </tbody>
        </table>
