@php
    ...
    $selectedEntrepriseId = old('entreprise_id', optional($formulaireExp42B)->entreprise_id ?? optional($dossier)->entreprise->id ?? '');
    $selectedDossierId = old('dossier_id', $dossier_id ?? $formulaireExp42B->dossier_id ?? '');
@endphp
                ...
                <tr>
                    ...
                    <td style="border:1px solid #000; padding:6px; vertical-align:top;">
                        <div style="margin-bottom:10px;">
                            ...
                            <input type="text" name="numero_marche" class="form-control" placeholder="Numéro de marché similaire" style="width:100%;" value="{{ old('numero_marche', optional($formulaireExp42B)->numero_marche ?? '') }}">
                        </div>
                        <textarea name="identification_marche" class="form-control" rows="3" placeholder="Identification du marché (Titre, détails...)" style="width:100%;">{{ old('identification_marche', optional($formulaireExp42B)->identification_marche ?? '') }}</textarea>
                        <div style="display:flex; gap:8px; margin-top:6px;">
                            <input type="date" name="date_attribution" placeholder="Date d'attribution" style="flex:1;" value="{{ old('date_attribution', optional($formulaireExp42B)->date_attribution?->format('Y-m-d') ?? '') }}">
                            <input type="date" name="date_achevement" placeholder="Date d'achèvement" style="flex:1;" value="{{ old('date_achevement', optional($formulaireExp42B)->date_achevement?->format('Y-m-d') ?? '') }}">
                        </div>
                    </td>
                    <td style="border:1px solid #000; padding:6px; vertical-align:top; text-align:center;">
                        <select name="role_marche" style="width:100%;">
                            ...
                            <option value="Fournisseur/Prestataire" {{ old('role_marche', optional($formulaireExp42B)->role_marche ?? '') === 'Fournisseur/Prestataire' ? 'selected' : '' }}>Fournisseur/Prestataire</option>
                            <option value="Ensemblier" {{ old('role_marche', optional($formulaireExp42B)->role_marche ?? '') === 'Ensemblier' ? 'selected' : '' }}>Ensemblier</option>
                            <option value="Sous-traitant" {{ old('role_marche', optional($formulaireExp42B)->role_marche ?? '') === 'Sous-traitant' ? 'selected' : '' }}>Sous-traitant</option>
                        </select>
                    </td>
                    <td style="border:1px solid #000; padding:6px; vertical-align:top; text-align:right;">
                        <input type="text" name="montant_total" placeholder="Montant" style="width:100%; text-align:right;" value="{{ old('montant_total', optional($formulaireExp42B)->montant_total ?? '') }}">
                        <input type="text" name="participation_pourcentage" placeholder="% participation" style="width:100%; margin-top:6px;" value="{{ old('participation_pourcentage', optional($formulaireExp42B)->participation_pourcentage ?? '') }}">
                        <input type="text" name="montant_part" placeholder="Montant part (auto)" style="width:100%; margin-top:6px; text-align:right;" readonly value="{{ old('montant_part', optional($formulaireExp42B)->montant_part ?? '') }}">
                        <input type="hidden" name="monnaie" value="{{ old('monnaie', optional($formulaireExp42B)->monnaie ?? 'FCFA') }}">
                    </td>
                    <td style="border:1px solid #000; padding:6px; vertical-align:top;">
                        <input type="text" name="autorite_nom" placeholder="Nom autorité" style="width:100%; margin-bottom:6px;" value="{{ old('autorite_nom', optional($formulaireExp42B)->autorite_nom ?? '') }}">
                        <input type="text" name="autorite_adresse" placeholder="Adresse" style="width:100%; margin-bottom:6px;" value="{{ old('autorite_adresse', optional($formulaireExp42B)->autorite_adresse ?? '') }}">
                        <input type="text" name="autorite_telephone" placeholder="Téléphone" style="width:100%; margin-bottom:6px;" value="{{ old('autorite_telephone', optional($formulaireExp42B)->autorite_telephone ?? '') }}">
                        <input type="text" name="autorite_email" placeholder="Email" style="width:100%;" value="{{ old('autorite_email', optional($formulaireExp42B)->autorite_email ?? '') }}">
                    </td>
